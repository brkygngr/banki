...
export const errorHandler: Middleware = (api: MiddlewareAPI) => (next) => (action) => {
  if (isRejectedWithValue(action)) {
    ...
    let response: ApiErrorResponse = {
      errors: [
        (action.meta.arg as { endpointName: string }).endpointName + ' ' + defaultErrorMessage,
      ],
      code: '',
    ...
    api.dispatch(
      showToast({
        message: response.code + ' ' + response.errors,
        durationMS: 6000,
        type: AppToastType.FAILURE,
        header: 'Failure',
      })
  ...
  if (isFulfilled(action)) {
    api.dispatch(
      showToast({
        message: action.type + ' ' + action.meta.arg,
        durationMS: 6000,
